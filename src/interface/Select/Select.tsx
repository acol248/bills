...
export default function Select({ className, items, label, placeholder, defaultValue, ...props }: Props) {
  const classList = useClassList({ defaultClass: "select", className, maps, string: true }) as string;

  return (
    <label className={mc("label")}>
      {label}
      <RadixSelect.Root {...props} defaultValue={defaultValue}>
        <RadixSelect.Trigger className={mc("select-trigger")}>
          <RadixSelect.Value placeholder={placeholder ?? "Select value"} />
          <RadixSelect.Icon>
